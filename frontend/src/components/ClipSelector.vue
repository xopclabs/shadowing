...
const visibleEnd = computed(() => Math.min(timelineOffset.value + visibleDuration.value, duration.value))
...
const positionToTime = (x: number, containerWidth: number): number => {
  ...
  return Math.max(0, Math.min(timelineOffset.value + relativeTime, duration.value))
}

// Seekbar helpers (full duration)
const timeToSeekbarPosition = (time: number, containerWidth: number): number => {
  return (time / duration.value) * containerWidth
}

const seekbarPositionToTime = (x: number, containerWidth: number): number => {
  return Math.max(0, Math.min((x / containerWidth) * duration.value, duration.value))
}
...
const onLoadedMetadata = () => {
  if (videoRef.value) {
    ...
    selectionEnd.value = Math.min(DEFAULT_SELECTION_DURATION, duration.value)
  }
}
...
const onSeekbarMouseMove = (e: MouseEvent) => {
  if (!isDragging.value || dragType.value !== 'seekbar' || !seekbarRef.value) return
...
const onTimelineWheel = (e: WheelEvent) => {
  ...
  const newVisible = Math.max(MIN_VISIBLE_DURATION, Math.min(MAX_VISIBLE_DURATION, currentVisible * (1 + delta)))
  ...
  if (newVisible !== currentVisible) {
    ...
    // Clamp offset
    timelineOffset.value = Math.max(0, Math.min(newOffset, duration.value - newVisible))
  }
}
...
const onTimelineMouseDown = (e: MouseEvent) => {
  ...
  const startPos = selectionStart.value !== null ? timeToPosition(selectionStart.value, rect.width) : null
  const endPos = selectionEnd.value !== null ? timeToPosition(selectionEnd.value, rect.width) : null
...
const timelineMarkers = computed(() => {
  ...
  let step: number
  if (visDur > 600) step = 60       // 1 minute
  else if (visDur > 300) step = 30  // 30 seconds
  else if (visDur > 120) step = 15  // 15 seconds
  else if (visDur > 60) step = 10   // 10 seconds
  else if (visDur > 30) step = 5    // 5 seconds
  else if (visDur > 10) step = 2    // 2 seconds
...
const centerTimelineOnPlayhead = () => {
  ...
  timelineOffset.value = Math.max(0, Math.min(currentTime.value - halfVisible, duration.value - visibleDuration.value))
}
  ...
  <div class="space-y-4">
    ...
    >
      <video
        ...
        @ended="isPlaying = false; isPreviewMode = false"
        @pause="onVideoPause"
        @play="onVideoPlay"
        preload="metadata"
        playsinline
      ...
      >
        <div class="w-16 h-16 rounded-full bg-sol-500/90 flex items-center justify-center shadow-lg">
          <svg class="w-8 h-8 text-noche-950 ml-1" fill="currentColor" viewBox="0 0 24 24">
    ...
    <div class="flex items-center justify-center gap-4">
      <button @click="skipBackward" class="p-3 rounded-xl bg-noche-800 hover:bg-noche-700 transition-colors">
        <svg class="w-6 h-6 text-noche-300" fill="currentColor" viewBox="0 0 24 24">
          <path d="M11 18V6l-8.5 6 8.5 6zm.5-6l8.5 6V6l-8.5 6z"/>
        </svg>
      </button>
      <button @click="togglePlay" class="p-4 rounded-xl bg-sol-500 hover:bg-sol-400 transition-colors">
        <svg v-if="!isPlaying" class="w-8 h-8 text-noche-950" fill="currentColor" viewBox="0 0 24 24">
          <path d="M8 5v14l11-7z" />
        </svg>
      ...
      <button @click="skipForward" class="p-3 rounded-xl bg-noche-800 hover:bg-noche-700 transition-colors">
        <svg class="w-6 h-6 text-noche-300" fill="currentColor" viewBox="0 0 24 24">
          <path d="M4 18l8.5-6L4 6v12zm9-12v12l8.5-6L13 6z"/>
        </svg>
      </button>
    ...
    <div class="space-y-2">
      <div class="flex items-center justify-between text-xs text-noche-500 px-1">
      ...
      >
        ...
        <div 
          ...
          :style="{ left: `${timeToPosition(marker.time, timelineRef?.clientWidth || 300)}px` }"
        >
          <span 
            v-if="marker.isMajor"
        ...
        >
          <!-- Start Handle -->
          <div class="absolute left-0 top-0 bottom-0 w-4 -ml-2 bg-sol-500 cursor-ew-resize flex items-center justify-center hover:bg-sol-400 transition-colors">
            <div class="w-0.5 h-6 bg-noche-950 rounded-full"></div>
          </div>
          
          <!-- End Handle -->
          <div class="absolute right-0 top-0 bottom-0 w-4 -mr-2 bg-sol-500 cursor-ew-resize flex items-center justify-center hover:bg-sol-400 transition-colors">
            <div class="w-0.5 h-6 bg-noche-950 rounded-full"></div>
          </div>
          
          <!-- Selection duration label -->
          <div class="absolute inset-0 flex items-center justify-center pointer-events-none">
            <span class="bg-sol-500 text-noche-950 text-xs font-bold px-2 py-0.5 rounded">
              {{ selectionDuration }}s
            </span>
          </div>
        </div>
        ...
        <div
          ...
          :style="{ left: `${timeToPosition(currentTime, timelineRef?.clientWidth || 300)}px` }"
        >
          <div class="absolute -top-1 left-1/2 -translate-x-1/2 w-3 h-3 bg-tierra-400 rounded-full"></div>
        </div>
    ...
    <!-- Selection info -->
    <div v-if="hasSelection" class="flex items-center justify-between text-sm bg-noche-900/50 rounded-lg p-3">
      <div class="space-y-1">
      ...
      <div class="text-right space-y-1">
        ...
        <div class="font-mono text-sol-400 font-bold">{{ selectionDuration }}s</div>
      </div>
    </div>
    ...
    <div class="flex gap-3">
      ...
      >
        <svg v-if="!isPreviewMode" class="w-4 h-4 mr-2 inline" fill="currentColor" viewBox="0 0 24 24">
          <path d="M12 5V1L7 6l5 5V7c3.31 0 6 2.69 6 6s-2.69 6-6 6-6-2.69-6-6H4c0 4.42 3.58 8 8 8s8-3.58 8-8-3.58-8-8-8z"/>
        </svg>
        <svg v-else class="w-4 h-4 mr-2 inline" fill="currentColor" viewBox="0 0 24 24">
          <rect x="6" y="4" width="4" height="16" rx="1" />
          <rect x="14" y="4" width="4" height="16" rx="1" />
        </svg>
