...
const startRecording = async () => {
  ...
  try {
    ...
    // Determine supported MIME type - use WAV for best spectrogram compatibility
    const mimeType = MediaRecorder.isTypeSupported("audio/wav")
      ? "audio/wav"
      : MediaRecorder.isTypeSupported("audio/webm;codecs=vorbis")
...
<template>
  <div class="space-y-4">
    ...
    <div
      v-if="error"
      class="p-3 rounded-lg bg-tierra-500/20 border border-tierra-500/30 text-tierra-300 text-sm"
    >
      {{ error }}
    </div>
    ...
    <div class="flex items-center justify-center gap-4">
      ...
      <button
        ...
        :class="
          isRecording
            ? 'bg-tierra-600 hover:bg-tierra-500'
