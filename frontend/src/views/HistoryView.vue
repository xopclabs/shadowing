...
<template>
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <h1 class="font-display text-2xl font-bold text-noche-100">
        History
      </h1>
    ...
    <div v-if="loading" class="text-center py-12">
      <div class="inline-block w-8 h-8 border-4 border-sol-500 border-t-transparent rounded-full animate-spin"></div>
      <p class="text-noche-400 mt-4">Loading...</p>
    </div>

    <!-- Error State -->
    <div v-else-if="error" class="card bg-tierra-500/10 border-tierra-500/30 text-center">
    ...
    <template v-else-if="activeTab === 'recordings'">
      ...
      <div v-if="recordings.length === 0" class="text-center py-12">
        <svg class="w-16 h-16 mx-auto text-noche-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" 
            d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z" />
        </svg>
        <p class="text-noche-400 mt-4">No recordings yet</p>
        <p class="text-noche-500 text-sm">Start a practice session to create your first recording</p>
      </div>
          ...
          <div class="flex items-start justify-between mb-3">
            <div>
              ...
              <div class="flex items-center gap-2 mt-1">
                <span class="text-xs text-noche-500 bg-noche-800 px-2 py-0.5 rounded">
                  Attempt #{{ recording.attempt_number }}
                </span>
                <span v-if="recording.clip_id" class="text-xs text-sol-500 bg-sol-500/10 px-2 py-0.5 rounded">
    ...
    <template v-else-if="activeTab === 'stats' && stats">
      ...
      <div class="card space-y-4">
        ...
        <div class="flex items-center justify-between py-2 border-b border-noche-800">
      ...
      <!-- Motivational Card -->
      <div class="card bg-gradient-to-br from-sol-500/20 to-tierra-500/20 border-sol-500/30">
        <div class="text-center">
          <p class="text-lg font-display font-semibold text-sol-300">
            {{ stats.recordings_this_week >= 7 ? '🔥 On Fire!' : 
               stats.recordings_this_week >= 3 ? '👍 Good Progress!' : 
               '💪 Keep Going!' }}
          </p>
